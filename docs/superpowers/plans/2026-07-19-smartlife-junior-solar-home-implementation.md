...
>
> 当前状态：任务一至十已完成，G1 软件模拟验收为 `mock-passed`；尚未完成 G2 电气预检、烧录或真板验收
...
## 25. 任务八执行结果（2026-07-19）

- 新增“节气智居 · 全屋运行台”，页面以两室两厅一卫剖面为核心，将主卧、次卧/书房、客厅、餐厅、卫生间及厨房、入户、阳台附区映射到现有固定 GPIO；页面没有评分化操作或可见文案。
- Web Serial 使用 `115200`，分别呈现串口连接、项目身份、真板/模拟来源和最新帧时间。只有先收到匹配 `smartlife-junior-solar-home-v1` 的 hello，才接受后续 telemetry/event/ack；坏 JSON 和错误项目不会修改最后有效状态。
- 超过 `3500ms` 没有新 hello/telemetry 后清除实时数据和告警结论；主动断开立即清空，不等待过期窗口。模拟帧持续显示“模拟数据，不代表实物动作”。
- T/Q/N/H、XN/YZ、节气参数、Auto/Sleep、房间状态和五项执行器目标均来自新遥测；MQ2 明确标为等效估算，声音不称 dB，PIR 不称识别人，继电器不称触点反馈。
- 告警纯核心只读取 `telemetry.alerts`，支持 MQ2/火焰/水浸/入侵的房间、GPIO、原因和实际目标动作摘要；未知代码保留可见，所有动态内容使用 `textContent`，不通过 `innerHTML` 插入协议数据。
- 网页命令使用唯一 ID；只收到 ACK 时显示“已接受，等待状态更新”，只有更新后 telemetry 的 `lastAppliedCommandId` 匹配才显示“状态已更新”。安全锁、延迟模式和超时均有独立文案。
- 浏览器模拟水浸、Sleep 命令闭环、停止遥测和主动断开均通过；Chromium 控制台 0 错误。390px 实测 `innerWidth=390`、页面与 body 的 `scrollWidth=390`。
- 网页协议测试 `9/9`、告警测试 `6/6`、页面合同和 11 帧模拟回放通过；Python 测试 `27/27`、固件原生测试 `24/24`、N16R8 编译继续通过，RAM `6.5%`、Flash `5.4%`。
- 本次没有选择真实串口、烧录固件或部署公网。任务九语音和任务十 G1 汇总尚未完成，因此项目状态仍低于 `mock-passed`。

## 26. 任务九执行结果（2026-07-19）

- 新增无 DOM 的语音意图纯核心，只接受“打开/切换到自动模式”和“打开/切换到睡眠模式”四句固定口令；空白、无关、含糊、否定、混合和危险话语全部返回 `unknown`，不能用更长句子包含白名单子串绕过检查。
- 白名单文字在生成命令时再次解析，合法结果只能成为 `origin=voice` 的单一 `mode=Auto/Sleep` 命令；语音不能关闭 MQ2、火焰、水浸或入侵判断，也不能直接控制风扇、继电器、RGB 或蜂鸣器。
- 全屋运行台新增 Chrome/Edge 浏览器语音入口、停止收音、识别状态、权限/无语音/无麦克风/网络错误说明和独立文字测试。文字测试明确只用于解析调试，不能代替麦克风与真板闭环证据。
- 语音命令沿用任务八唯一 ID 事务：只收到 ACK 不显示完成；ACK 被拒绝时保留原模式；只有带相同 `lastAppliedCommandId` 的新遥测到达才显示闭环完成。
- P1 安全期间的模式命令继续服从固件合同：`deferredBy=safety` 显示模式已记录，匹配新遥测后仍明确显示“P1 安全联动继续”，不会把模式更新误写成安全输出被覆盖。
- 浏览器验证了危险话语零发送、模拟麦克风事件、普通 Auto/Sleep 闭环、ACK 拒绝保持原模式和水浸 P1 延迟模式。390px 无横向溢出，Chromium 控制台 0 错误、0 警告。
- 语音意图测试 `10/10`、网页协议测试 `9/9`、告警测试 `6/6`、页面合同和 11 帧模拟回放通过；Python 测试 `27/27`、固件原生测试 `24/24`、N16R8 编译通过，RAM `6.5%`、Flash `5.4%`。
- 本次没有采集真人麦克风音频、选择真实串口、烧录固件或部署公网。任务十完整 G1 汇总尚未完成，因此项目状态仍低于 `mock-passed`。

## 27. 任务十执行结果（2026-07-20）

- 新增可重复运行的 G1 证据生成器和 `2/2` 专项测试；生成 23 条模拟主板帧、4 条外发命令和 12 个连续遥测序号，原始 JSONL 与结构化结论分文件保存。
- 九类场景检查全部通过：身份、Auto/Sleep、GPIO17 跨阈值、小暑/大寒、水浸触发与三秒恢复、命令 ACK 加匹配 telemetry、3500/3501ms 离线边界、模拟来源和遥测序号。
- Chromium 重放同一份 23 行主板输出，逐帧观察到 Auto/Sleep、阈值 `18/35℃`、风扇 `100/0%`、小暑 `80/30`、大寒 `20/45`、水浸 P1 和 Sleep `35%` 恢复；停止输入 `4200ms` 后页面离线并清空实时值。
- 390px 页面与 body 均无横向溢出，Chromium 控制台 0 错误、0 警告。
